...
function CameraRig() {
  const controls=useRef<OrbitControlsImpl>(null); const followed=useTownStore(s=>s.followedBotId); const focused=useTownStore(s=>s.selectedLocationId); const {camera}=useThree();
...
export function BotTownScene(){ if(typeof document!=="undefined"&&!webglAvailable())return <div className="webgl-fallback"><strong>Bot Town needs WebGL</strong><span>Enable hardware acceleration or try a modern browser to enter the town.</span></div>;
  return <Canvas shadows dpr={1} gl={{antialias:true,powerPreference:"high-performance"}} onPointerMissed={()=>useTownStore.getState().selectBot(null)}><Suspense fallback={null}><SceneContents/></Suspense></Canvas>;
}